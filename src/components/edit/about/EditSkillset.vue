    ...
    <v-col cols="12" md="10" offset-md="1">
      ...
      <base-info-card title="Preview" color="yellow">
        <v-card light class="px-4 pt-6" rounded='xl' style="padding-bottom:8rem" flat>
          <v-col cols="12" md="10" offset-md="1" class="pa-0">
            <news-skillset ref="preview" :edit="true" :field="field" :tech="tech" :other="other"/>
          </v-col>
        </v-card>
      </base-info-card>
    </v-col>
...
export default {
  ...
  computed: {
    skillset_m: {
      ...
      set: function (value) {
        this.field = value.field.concat()
        this.tech = value.tech.concat()
        this.other = value.other.concat()
      }
    }
  ...
  methods: {
    add() {
      ...
      if(this.$refs.form.validate()){
        ...
        let item = this.name
        if (this.skillset_m[this.type].length >= 9) {
          this.$emit('message', 'can not add more than 9 items in a field', 'warn')
          return 
        }
        if(preview.addItem(preview[this.type], item)) {
